...
AI 모델이 정교해지려면 파라미터를 키울 수밖에 없는데, 모든 연산을 디바이스 안에서 처리하는 것은 전력·발열·메모리 측면에서 한계가 명확하다. 그렇다면 **디바이스는 입출력(디스플레이·센서)을 담당하고, 무거운 연산은 물리적으로 가까운 소규모 고성능 엣지서버가 맡아 결과를 통신으로 돌려주는** 3-티어 구조(디바이스 ↔ 엣지서버 ↔ 클라우드)가 효율적일 수 있다. 엣지서버는 충분한 쿨링과 고성능 AI 칩을 갖춰 디바이스보다 압도적으로 빠르게 연산한다.
